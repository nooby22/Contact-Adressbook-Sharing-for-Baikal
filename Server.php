...
#
#  MODIFICATIONS (GPLv2 Section 2a compliance):
#  2026-01-03: Modified by noob22
#  - Replaced standard CardDAV PDO backend with SharedReadonlyCardDAVBackend.
#  - Added 'beforeMethod' event listener to inject the current principal 
#    into the CardDAV backend for shared address book access control.
#  - Updated CardDAV plugin initialization to use the modified backend.
#
...
use Baikal\Core\SharedReadonlyCardDAVBackend;
...
class Server {
    ...
    protected function initServer() {
		...
		/* Wurde inaktiviert und durch die nachfolgende Modifikation1 ersetzt, mit nur dieser Änderung läuft alles
		...
		*/
		// --- START MODIFIKATION1 ---
        if ($this->enableCardDAV) {
            // Wir nutzen hier Ihr neues SharedReadonly-Backend
            $carddavBackend = new \Baikal\Core\SharedReadonlyCardDAVBackend($this->pdo);
            $nodes[] = new \Sabre\CardDAV\AddressBookRoot($principalBackend, $carddavBackend);
        }
        // --- ENDE MODIFIKATION1 ---
        ...
        if ($this->enableCardDAV) {
			// WICHTIG: Den Injektor erst registrieren, nachdem $this->server existiert!
            $this->server->on('beforeMethod', function($request, $response) use ($carddavBackend) {
                $authPlugin = $this->server->getPlugin('auth'); // Korrekt über $this->server
                if ($authPlugin) {
                    $currentPrincipal = $authPlugin->getCurrentPrincipal();
                    if ($currentPrincipal) {
                        $carddavBackend->setCurrentUser($currentPrincipal);
                    }
                }
            });
																
            $this->server->addPlugin(new \Sabre\CardDAV\Plugin($carddavBackend));
